...
function LoginContent() {
  ...
  return (
    <div
      ...
      className="min-h-screen grid grid-cols-1 lg:grid-cols-12 bg-[#F8F9FE] text-slate-900 font-sans selection:bg-[#3B25B0] selection:text-white"
    >

      {/* Left Column - Deep Blue Gradient Branding Banner */}
      <div className="lg:col-span-5 xl:col-span-5 bg-gradient-to-br from-[#20107A] via-[#2A179E] to-[#3B25B0] text-white p-8 sm:p-12 lg:p-16 flex flex-col justify-between relative overflow-hidden min-h-[400px] lg:min-h-screen">
        ...
        {/* Top Logo - Using /images/logo_footer.png */}
        ...
        <div className="relative z-10 my-auto py-10 max-w-md">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight leading-[1.15] text-white mb-6">
            {currentLang === "ar" ? (
              <>
        ...
        <div className="relative z-10 flex flex-wrap gap-2.5 sm:gap-3 text-xs font-medium text-white/90 pt-4">
      ...
      {/* Right Column - Form Container Area (Increased Width max-w-xl) */}
      <div className="lg:col-span-7 xl:col-span-7 flex flex-col justify-between p-4 sm:p-8 lg:p-12 relative bg-[#F8F9FE]">

        {/* Top Header Row (Back Link & Language Model Switcher) */}
        <div className="flex items-center justify-between w-full max-w-xl mx-auto mb-6 sm:mb-8">
          <Link
            href="/"
          ...
          <button
            ...
            className="inline-flex items-center gap-2 bg-white border border-slate-200 shadow-xs hover:border-indigo-300 text-slate-700 hover:text-[#3B25B0] text-xs font-bold px-3.5 py-2 rounded-xl transition-all"
          >
        ...
        {/* Main Sign In Form Card (Increased max-w-xl / ~540px width) */}
        <div className="w-full max-w-xl mx-auto bg-white rounded-3xl p-6 sm:p-10 shadow-xl border border-slate-100 my-auto relative">

          {/* SECTION 1: NAFATH LOGIN (Image 1) */}
          {step === "nafath" && (
            <div className="animate-in fade-in duration-300">
              {/* Card Header (Removed right badge inside card header) */}
              <div className="mb-6">
              ...
              {/* Segmented Tabs */}
              <div className="bg-slate-100/80 p-1 rounded-2xl grid grid-cols-2 text-xs sm:text-sm font-semibold mb-6">
                <button
                  type="button"
                  className="py-2.5 rounded-xl bg-white text-[#3B25B0] shadow-sm text-center transition-all"
                >
                  {t("nafathTab")}
                </button>
                <button
                  ...
                  className="py-2.5 rounded-xl text-slate-500 hover:text-slate-900 text-center transition-all"
                >
                  {t("emailTab")}
                </button>
              </div>

              <form onSubmit={handleNafathSubmit} className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-4 py-3.5 rounded-xl border border-slate-200 text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#3B25B0] focus:border-transparent transition-all placeholder:text-slate-400"
                  />
                  <p className="text-[11px] text-slate-400 mt-1.5">{t("nationalIdHelper")}</p>
                </div>

                <button
                  type="submit"
                  className="w-full bg-[#0D9488] hover:bg-[#0B8479] text-white py-3.5 rounded-xl font-semibold text-sm shadow-md hover:shadow-emerald-200/50 transition-all flex items-center justify-center gap-2 transform active:scale-[0.99]"
                >
                  {/* Nafath Arabic Icon Box */}
                  <span className="w-5 h-5 rounded-md bg-white/20 text-white flex items-center justify-center font-bold text-xs">
                    ن
                  </span>
              ...
              <div className="relative my-6 text-center">
                ...
                <span className="relative bg-white px-3 text-xs text-slate-400 font-medium">{t("orText")}</span>
              </div>
              ...
              <div className="text-center">
                <button
                  type="button"
                  onClick={() => setStep("email")}
                  className="text-xs sm:text-sm font-bold text-[#3B25B0] hover:text-[#2F1F99] transition-colors"
                >
                  {t("useEmailInstead")}
                </button>
              </div>
          ...
          {/* SECTION 2: EMAIL & PASSWORD LOGIN (Image 2) */}
          {step === "email" && (
            <div className="animate-in fade-in duration-300">
              {/* Card Header (Removed right badge inside card header) */}
              <div className="mb-6">
              ...
              <form onSubmit={handleEmailSubmit} className="space-y-4">
                ...
                <button
                  type="submit"
                  className="w-full bg-[#3B25B0] hover:bg-[#2F1F99] text-white py-3.5 rounded-xl font-semibold text-sm shadow-md hover:shadow-indigo-300/50 transition-all text-center mt-2 transform active:scale-[0.99]"
                >
                  {t("signIn")}
                </button>
              </form>
          ...
          {/* SECTION 3: WAITING FOR NAFATH APPROVAL (Image 3) */}
          {step === "nafath_pending" && (
            <div className="text-center py-4 animate-in fade-in duration-300">
              ...
              {/* Status Pill Badge */}
              <div className="inline-flex items-center gap-2 bg-emerald-100/80 text-emerald-800 border border-emerald-200/80 text-xs font-bold px-3.5 py-1.5 rounded-full mb-6">
              ...
              <div className="my-6">
                <span className="text-6xl sm:text-7xl font-extrabold tracking-tight text-[#3B25B0] inline-block font-mono">
                  32
                </span>
              </div>
          ...
          {/* SECTION 4: IDENTITY VERIFIED (Image 4 - Uses /images/success.png) */}
          {step === "nafath_verified" && (
            <div className="text-center py-4 animate-in fade-in duration-300">
              ...
              {/* Success Image (/images/success.png) replaced as requested */}
              ...
              <button
                ...
                className="w-full bg-[#3B25B0] hover:bg-[#2F1F99] text-white py-3.5 rounded-xl font-semibold text-sm shadow-md hover:shadow-indigo-300/50 transition-all text-center"
              >
                {t("goToDashboard")}
              </button>
            </div>
          ...
          <div className="mt-8 pt-6 border-t border-slate-100 text-center">
            ...
            <div className="flex items-center justify-center gap-4 text-[10px] font-extrabold text-slate-400 uppercase tracking-widest">
              <span>IA</span>
              <span>NCA ECC</span>
              <span>PDPL</span>
            </div>
