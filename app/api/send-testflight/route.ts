...
export async function POST(req: Request) {
  try {
    ...
    // 1. Record request locally in testflight_requests.json
    try {
      const logFilePath = path.join(process.cwd(), "..", "testflight_requests.json");
      let existingLogs = [];
      ...
      existingLogs.push({
        testerEmail,
        targetEmail: "anshad@nexbex.in",
        senderEmail: "sahl.nexbex@gmail.com",
        timestamp: formattedDate,
      });
      fs.writeFileSync(logFilePath, JSON.stringify(existingLogs, null, 2));
    } catch (logErr) {
      console.warn("Failed to write to local log file:", logErr);
    }

    // 2. Attempt Nodemailer SMTP Dispatch
    let emailSent = false;
    let emailError = "";
    ...
    try {
      ...
      console.warn("SMTP send failed (logged request locally):", err?.message || err);
      emailError = err?.message || "SMTP Auth failed";
    }
    ...
    return NextResponse.json(
      {
        ...
        emailSent,
        message: emailSent
          ? "TestFlight request email sent directly to anshad@nexbex.in"
          : "TestFlight request recorded for anshad@nexbex.in",
        testerEmail,
      },
      {
        status: 200,
